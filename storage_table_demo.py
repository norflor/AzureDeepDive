...
# Define variables with random resource group and storage account names.  Adding additional commentary to test.
resourcegroup_name = 'nmf'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'nmf'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
location = 'westus2'
...
coffee.flavor = 'punkin spice latte'
coffee.cupsize = 'tall'
coffee.price = 4.25
...
coffee.PartitionKey = 'Coffeemenu'
coffee.RowKey = '006'
coffee.brand = 'CoffeeBean'
coffee.flavor = 'mocha latte'
coffee.cupsize = 'regular'
coffee.price = 4.29
table_service.insert_entity('itemstable', coffee)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'Cargallery'", select='make,model,year,color,price')
for item in items:
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'Coffeemenu'", select='brand,flavor,cupsize,price')
for item in items:
